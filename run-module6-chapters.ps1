<#
Module 6 - Chained Chapter Runner (v1)

Runs MODULE_6_CHAPTER_<N>_REBUILD_PROMPT.md through the Claude Code CLI in
sequence, one chapter at a time, mirroring run-module5-chapters.ps1. Each
chapter's brief assumes the previous chapter's file already exists in the
repo (Chapter 1 also scaffolds the `Module 6/` app itself).

WHY THE SKIP GUARD AND BASELINE COMMIT EXIST (same rationale as Module 5's
runner):

  1. SKIP GUARD. Before invoking Claude Code for chapter N, the script checks
     whether Module 6/src/content/chapterN.ts already exists - if it does,
     ...
     left off. It also means running this script after Module 6/ is fully
     built is a safe no-op.

  2. BASELINE COMMIT. The Module 6 syllabus and all eight
     MODULE_6_CHAPTER_N_REBUILD_PROMPT.md briefs were written directly to
...
source files (see MODULE_6_CHAPTER_8_REBUILD_PROMPT.md section 5). If you
...
USAGE - run from a normal PowerShell/VS Code terminal on THIS machine:
    ...
    .\run-module6-chapters.ps1                # runs 1-8, skips what already exists
    .\run-module6-chapters.ps1 -Start 4 -End 4 # force just one chapter (delete its file first to force a real regen)

REQUIREMENTS:
  - Claude Code CLI ('claude') on PATH and already logged in.
  - Run from your own terminal with network access.
  - `npm` and `git` on PATH.
  - Module 6/'s node_modules isn't included in what gets scaffolded by
...
$moduleDir = Join-Path $repoRoot "Module 6"

Write-Host "Module 6 chained runner: Chapters $Start-$End" -ForegroundColor Yellow
Write-Host "Repo root: $repoRoot`n"

if (-not (Test-Path (Join-Path $moduleDir "package.json"))) {
    Write-Host "'Module 6' app isn't scaffolded yet - that's fine, Chapter 1's brief scaffolds it from scratch (the folder may already exist with just a placeholder README.md in it, which is fine too)." -ForegroundColor DarkYellow
} elseif (-not (Test-Path (Join-Path $moduleDir "node_modules"))) {
    Write-Host "=== Module 6/node_modules missing - running npm install once ===" -ForegroundColor Cyan
...
if ($preExistingChanges) {
    ...
    git add -A
    git commit -m "Module 6: baseline snapshot before chained runner (syllabus + chapter briefs)" | Out-Null
    if ($LASTEXITCODE -ne 0) {
...
for ($n = $Start; $n -le $End; $n++) {
    ...
    if (Test-Path $chapterFile) {
        Write-Host "=== Chapter $n : Module 6/src/content/chapter$n.ts already exists - skipping (no API call, no git touch) ===" -ForegroundColor DarkGray
        continue
    }

    $promptFile = Join-Path $repoRoot "MODULE_6_CHAPTER_${n}_REBUILD_PROMPT.md"
    ...
    $logFile = Join-Path $repoRoot "module6-chapter$n-run.log"
    ...
    if (-not $selfCommitted -and -not $changes) {
        ...
        Write-Host "Check $logFile for what it actually said, then retry with: .\run-module6-chapters.ps1 -Start $n -End $n" -ForegroundColor Red
        break
    }
    ...
    if ($buildExit -ne 0) {
        ...
        if ($selfCommitted) {
            ...
            Write-Host "Fix Module 6/ manually, or re-run just this chapter with: .\run-module6-chapters.ps1 -Start $n -End $n" -ForegroundColor Red
        }
        break
    }
    ...
    if ($selfCommitted) {
        ...
        git add -A
        git commit -m "Module 6 Chapter $n (auto-built via chained script)" | Out-Null
        if ($LASTEXITCODE -ne 0) {
...
Write-Host "If every chapter was skipped, Module 6/ was already complete - nothing to do." -ForegroundColor Yellow
