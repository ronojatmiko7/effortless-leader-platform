<#
Module 7 - Chained Chapter Runner (v2 - script-based scaffolding)

Runs MODULE_7_CHAPTER_<N>_REBUILD_PROMPT.md through the Claude Code CLI in
sequence, one chapter at a time, mirroring run-module6-chapters.ps1 with one
deliberate change: this script scaffolds "Module 7/" itself via
scaffold-module.ps1 (copy + find/replace) BEFORE calling Claude Code for
Chapter 1, instead of having Chapter 1's brief instruct Claude Code to
manually Read+Write-port ~15 shared primitive files. That porting never
needed an LLM - it was the single biggest token cost in earlier modules'
Chapter-1 build runs. See scaffold-module.ps1's header comment for the full
rationale.

Each chapter's brief still assumes the previous chapter's file already
exists in the repo. Chapter 1's brief now only covers chapter1.ts content +
registering it in chapters.ts (the scaffold is a precondition it verifies,
not a task it performs).

WHY THE SKIP GUARD AND BASELINE COMMIT EXIST (same rationale as Module 6's
runner):

  1. SKIP GUARD. Before invoking Claude Code for chapter N, the script checks
     whether Module 7/src/content/chapterN.ts already exists - if it does,
     ...
     left off. It also means running this script after Module 7/ is fully
     built is a safe no-op.

  2. BASELINE COMMIT. The Module 7 syllabus, all eight
     MODULE_7_CHAPTER_N_REBUILD_PROMPT.md briefs, and this script itself
...
source files (see MODULE_7_CHAPTER_8_REBUILD_PROMPT.md section 5). If you
...
USAGE - run from a normal PowerShell/VS Code terminal on THIS machine:
    ...
    .\run-module7-chapters.ps1                # scaffolds if needed, runs 1-8, skips what already exists
    .\run-module7-chapters.ps1 -Start 4 -End 4 # force just one chapter (delete its file first to force a real regen)
    .\run-module7-chapters.ps1 -Model claude-sonnet-4-5   # pin a specific model instead of the CLI default
...
REQUIREMENTS:
  ...
  - Module 7/'s node_modules isn't included in what scaffold-module.ps1
...
$moduleDir = Join-Path $repoRoot "Module 7"

Write-Host "Module 7 chained runner: Chapters $Start-$End" -ForegroundColor Yellow
...
if (-not (Test-Path (Join-Path $moduleDir "package.json"))) {
    Write-Host "=== 'Module 7/' isn't scaffolded yet - running scaffold-module.ps1 ===" -ForegroundColor Cyan
    ...
    & $scaffoldScript -SourceModule 6 -TargetModule 7 `
    ...
    Write-Host "'Module 7/package.json' already exists - scaffold already done, skipping scaffold-module.ps1." -ForegroundColor DarkGray
}

if (-not (Test-Path (Join-Path $moduleDir "node_modules"))) {
    Write-Host "=== Module 7/node_modules missing - running npm install once ===" -ForegroundColor Cyan
...
if ($preExistingChanges) {
    ...
    git add -A
    git commit -m "Module 7: baseline snapshot before chained runner (scaffold + syllabus + chapter briefs)" | Out-Null
    if ($LASTEXITCODE -ne 0) {
...
for ($n = $Start; $n -le $End; $n++) {
    ...
    if (Test-Path $chapterFile) {
        Write-Host "=== Chapter $n : Module 7/src/content/chapter$n.ts already exists - skipping (no API call, no git touch) ===" -ForegroundColor DarkGray
        continue
    }

    $promptFile = Join-Path $repoRoot "MODULE_7_CHAPTER_${n}_REBUILD_PROMPT.md"
    ...
    $logFile = Join-Path $repoRoot "module7-chapter$n-run.log"
    ...
    if (-not $selfCommitted -and -not $changes) {
        ...
        Write-Host "Check $logFile for what it actually said, then retry with: .\run-module7-chapters.ps1 -Start $n -End $n" -ForegroundColor Red
        break
    }
    ...
    if ($buildExit -ne 0) {
        ...
        if ($selfCommitted) {
            ...
            Write-Host "Fix Module 7/ manually, or re-run just this chapter with: .\run-module7-chapters.ps1 -Start $n -End $n" -ForegroundColor Red
        }
        break
    }
    ...
    if ($selfCommitted) {
        ...
        git add -A
        git commit -m "Module 7 Chapter $n (auto-built via chained script)" | Out-Null
        if ($LASTEXITCODE -ne 0) {
...
Write-Host "If every chapter was skipped, Module 7/ was already complete - nothing to do." -ForegroundColor Yellow
