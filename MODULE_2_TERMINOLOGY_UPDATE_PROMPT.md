# Build Brief: Module 2 — Rename Lead/Lag Metric to KPI Usaha / KPI Hasil
...
`Module 2/` ("KPI Architecture & Strategic Cascading") is a React card-deck
micro-learning app. Its core teaching, spanning Chapters 3 through 8, is the
classic predictive-vs-result KPI distinction, currently taught under the
English-loanword labels **"Lag Metric"** and **"Lead Metric"**:

- **"Lag Metric"** is formally introduced in `Module 2/src/content/chapter3.ts`,
  starting at card `c3-card-1` (eyebrow: `"Bab 3 · Lag Metric"`) — the result
  number that's already happened, the rearview mirror.
- **"Lead Metric"** is formally introduced in `Module 2/src/content/chapter4.ts`,
  starting at card `c4-card-1` (eyebrow: `"Bab 4 · Lead Metric"`) — the daily
  action that's predictive and controllable, the steering wheel.
...
Confirmed via grep: `chapter3.ts`, `chapter4.ts`, `chapter5.ts`, `chapter6.ts`,
`chapter7.ts`, `chapter8.ts` all reference "Lead" and/or "Lag" repeatedly.
Chapters 1 and 2 don't use these exact labels (they set up the underlying
idea informally first — chapter3 card `c3-card-2` even says "Di Bab 2 Anda
sudah kenal Lag Metric", referring back to that informal setup).
...
- [ ] "KPI Hasil" introduced with its adapted framing sentence at the point
      "Lag Metric" is first named in `chapter3.ts`
- [ ] "KPI Usaha" introduced with its adapted framing sentence at the point
      "Lead Metric" is first named in `chapter4.ts`
...
- [ ] Runs via `npm run build` with no errors inside `Module 2/`
